...
async function upsertSchools(schools) {
  try {
    // Use individual updates instead of batch upsert to avoid constraint issues
    for (const school of schools) {
      ...
      if (updateError && updateError.code === 'PGRST116') {
        ...
        if (insertError) {
          console.error(`Error inserting school ${school.urn}:`, insertError);
        }
      } else if (updateError) {
        console.error(`Error updating school ${school.urn}:`, updateError);
      }
    }
  } catch (err) {
...
async function main() {
  console.log('🚀 Starting COMPREHENSIVE primary results import...');
  console.log('This will import correct data for ALL schools');
  console.log('Using verified column positions for each year');
  console.log('Processing in small batches for reliability...');
